...
int main(int argc, char* argv[])
{
	...
	const char* login_params = "appid = 5d15e637, work_dir = .";
...
/*
	...
	if(aud_src != 0) {
		...
		demo_file("wav/iflytek01.wav", session_begin_params); 
	}
*/
	...
	json_buf = creat_json_buf(g_result);
	if(json_buf){
	...
	const char* tts_params = "voice_name = xiaoyan, text_encoding = utf8, sample_rate = 16000, speed = 50, volume = 50, pitch = 50, rdn = 2";
	if(cmd[0] == 0){
		printf("parse json failed\n");
		goto exit;
	}
	ret = text_to_speech(cmd, filename, tts_params);
	if (MSP_SUCCESS != ret)
	{
